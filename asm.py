...
def illegal_instr_handl(x):
    if x[0] not in ['add', 'sub', 'mov', 'ld', 'st', 'mul', 'div', 'rs', 'ls', 'xor', 'or', 'and', 'not', 'cmp', 'jmp', 'jlt', 'jgt', 'je', 'hlt','var']:
    ...
    if len(i) == 1:
        ...
        if i[0] == 'mov':
            ...
            if i[0] in ['div', 'not', 'cmp']:
                string = opCodes[i[0]]+'00000' + \
                    registers[i[1]]+registers[i[2]]+'\n'
                sys.stdout.write(string)
            else:
                if i[0] in ['ls', 'rs']:
                    string = opCodes[i[0]]+'0' + \
                        registers[i[1]]+f'{int(i[2][1:]):07b}'+'\n'
                    sys.stdout.write(string)
                elif i[0] in ['ld', 'st']:
                    string = opCodes[i[0]]+'0' + \
                        registers[i[1]]+var_dict[i[2]]+'\n'
                    sys.stdout.write(string)
    elif len(i) == 4:
        string = opCodes[i[0]]+'00'+registers[i[1]] + \
